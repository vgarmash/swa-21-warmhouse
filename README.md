...
В предложенной архитектуре системы умного дома микросервисам облачной системы нет необходимости взаимодействовать друг с другом. Фронт-енд приложение будет вызывать микросервисы через REST API и объединять результаты вызовов сервисов с помощью обмена состоянием через EventBus.

### 2. Документация API

Для примера смотрите документацию API:
[Smart House Device Management API](schemas/api/device_service_api.json)

# Задание 5. Работа с docker и docker-compose

Перейдите в apps/task5.
...
Ревьюер будет проверять точно так же. (Смотрите в [task5](apps/task5))
